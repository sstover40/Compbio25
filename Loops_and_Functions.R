...
for (i in seq_along(z)) {
  if(i %% 2==0) next
  #if the value of i is divisible by 2, skip to the NEXT i and restart loop
  print(i)
}
#do we actually need to write a loop here though? see below!
...
#       n1 = initial pop size (=n[1])
#       lambda = finite rate of increase
#       noise_sd = sd of a normal distribution with mean 0
#output: vector n with pipulation size >0
#       until extinction, then N/A
#-------------------------------------------------
library(ggplot2)
...
#noise turned off, confirming that the code is doing what it should be
qplot(x=1:100,y=ran_walk(noise_sd=0),geom="line")

#moving away from default lambda value, exponential growth curve
...
#creating a dataframe with parameter combos
...
#now we can slice and dice it any way you want
p2 <- p1
